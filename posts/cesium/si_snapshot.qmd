...
async function dbquery(q, params=[]) {
    const cn = await DuckDB.connect();
    const query = cn.prepare(q);
    const results = query.query(...params);
    cn.close();
    return results;
}

async function* dbqueryGenerator(q, params=[]) {
    const cn = await DuckDB.connect();
    const query = cn.prepare(q);
    for await (const batch of await query.send(...params)) {
